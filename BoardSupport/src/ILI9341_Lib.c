...
#include "G8RTOS_Lab4/G8RTOS_CriticalSection.h"
 ...
 * Write to TFT CS (PE0)
 */
void WriteTFT_CS(uint8_t value)
{
 ...
 */
void WriteTFT_DC(uint8_t value){
...
{
	LCD_SetAddress(Ypos, Xpos, Ypos+1, Xpos+1);
}
...
void LCD_SetAddress(uint16_t x1,uint16_t y1,uint16_t x2,uint16_t y2)//set coordinate for print or other function
{
	...
	LCD_WriteData(x1>>8); //Start Column
	LCD_WriteData(x1);
	LCD_WriteData(x2>>8); //End Column
	LCD_WriteData(x2);
	LCD_WriteIndex(0x2B); //Page Address Set
	LCD_WriteData(y1>>8); //Start Page
	LCD_WriteData(y1);
	LCD_WriteData(y2>>8); //End Page
	LCD_WriteData(y2);
...
{
	LCD_SetAddress(0, 0, MAX_SCREEN_Y, MAX_SCREEN_X);
	WriteTFT_CS(0);
	WriteTFT_DC(1);
	int i;
...
void LCD_DrawRectangle(uint16_t x,uint16_t y,uint16_t w,uint16_t h,uint16_t color)
{
	...
	//boundaries of LCD
	if(x > MAX_SCREEN_X) 	x = MAX_SCREEN_X;
	if(y > MAX_SCREEN_Y) 	y = MAX_SCREEN_Y;
	if(x2 > MAX_SCREEN_X) 	x2 = MAX_SCREEN_X;
	if(y2 > MAX_SCREEN_Y) 	y2 = MAX_SCREEN_Y;
